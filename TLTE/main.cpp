...
void drawScene() {
	...
	cout << "pos y: " << y << endl;
	cout << "pos z: " << z << endl;
	cout << "rot x: " << rotationx << endl;
	cout << "rot y: " << rotationy << endl;
	...
	glTranslatef(-cam.GetPos().x, -cam.GetPos().y, -cam.GetPos().z);//takes camera movement into calc
	
	if (cam.GetX() > 0) { glRotatef(cam.GetX(), 1, 0, 0); }
	if (cam.GetY() > 0) { glRotatef(cam.GetY(), 0, 1, 0); }
...
void update(int value) {
	...
	advance(_guys,
		...
		_numCollisions);
	float zpos = -(cam.GetPos().z);
	height = heightAt(_terrain, cam.GetPos().x, zpos);
	width = _terrain->width();
	length -= _terrain->length();
	//if (cam.GetPos().x >0 && cam.GetPos().x<width)
	//{
		//if (cam.GetPos().z < 0 && cam.GetPos().z < length)
		//{
...
static void SpecialMouseCB(int x, int y)
{
	cam.OnMouse(x,y);
}
...
int main(int argc, char** argv) {
	...
	glutCreateWindow("Lost Treasure Engine");
	initRendering();

	_terrain = loadTerrain("heightmap.bmp", 10.0f); //Load the terrain
